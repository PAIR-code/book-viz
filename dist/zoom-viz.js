...
let currentLevel;
export function drawZoomPlot(data, text, chapterBreaks) {
    const num_levels = 3;
    const height = 100, width = 700;
    ...
    clusterSvg.selectAll()
        ...
        .enter()
        .append("rect")
        .attr("x", function (d) { return x(d.x_start) + x(0.25); })
        .attr("y", function (d) { return y(1); })
        .attr("width", function (d) { return Math.max(0, x(d.section_length) - x(0.5)); })
    ...
    clusterSvg.call(d3.zoom().on("zoom", function () {
        ...
        if (scale > 1.2) {
            ...
            const sectionsInDomain = data.filter(function (entry) {
    ...
    contextSvg.selectAll()
        ...
        .enter()
        .append("rect")
        .attr("x", function (d) { return x(d.chapter_start) + x(0.5); })
        .attr("y", contextHeight * 0.15)
        .attr("width", function (d) { return Math.max(0, x(d.chapter_length) - x(1)); })
...
//# sourceMappingURL=zoom-viz.js.map