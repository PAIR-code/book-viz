(function (factory) {
  ...
  function transformer() {
    ...
    scale.domain = function(_) {
      return arguments.length ? (domain = map$2.call(_, number), clamp === identity$3 || (clamp = clamper(domain)), rescale()) : domain.slice();
    };
   ...
   * */
  function getParData(paragraphs, chapterBreaks, embeddings, order, dimensions) {
      if (order == 'color') {
  ...
  function arrayToRGBAString(arr, opacity = 0.8) {
   ...
   */
  const rectHeight = 4;
  function drawGridPlot(bookIdx, embData, embLength, splitBy, dimensions, order, data) {
      ...
      // set up svg
      const chapterPlotSvg = select('#visualizationBody')
      ...
      const parData = getParData(textChunks, chapterBreaks, embs, order, dimensions);
      const binFunc = histogram()
          .domain([0, nbins])
          .thresholds(nbins)
          .value(d => d.value);
      ...
      const totalCols = sum(colsPerBin) + colsPerBin.length - 1;
      const rectWidth = width / totalCols;
      const binXPositions = colsPerBin.map((col, idx, arr) => sum(arr.slice(0, idx)) + idx);
      const xScale = linear$1().domain([0, totalCols + 1]).range([0, width]);
      ...
      $('#searchText').on('input', (ev) => {
          ...
          if (!(searchStr) || (searchStr.length == 0)) {
              selectAll('rect').style('opacity', 1.0);
          }
          else {
              selectAll('rect')
                  .filter(function (d, i) {
  ...
  function tooltipOn(d, i, n, dimensions) {
      ...
      let gY = currentElY;
      select(currentEl).classed('selected', true);
      select(currentEl)
          .transition()
          .duration(200)
  ...
  function tooltipOff(d, i, n) {
      const currentEl = n[i];
      select(currentEl).classed('selected', false);
      select(currentEl)
          .transition()
          .duration(200)
          .attr('height', rectHeight - 1)
  ...
  function embeddingToRGBA(embRow, dimensions) {
      if (embRow.length == 1) {
          ...
          return arrayToRGBAString$1(newEmbRow);
      }
      if (dimensions === 'all') {
          embRow.push(1);
          return arrayToRGBAString$1(embRow);
      }
      else {
  ...
  function arrayToRGBAString$1(arr) {
   ...
   * limitations under the License.
   */
  function drawClusterPlot(data, text, chapterBreaks) {
      const num_levels = 3;
      const height = 200, width = 700;
      const contextHeight = 15;
      const margin = { top: 10, right: 10, bottom: 10, left: 10 };
      // set up svg
      const mainSvg = select('#visualizationBody')
          .classed("svg-container", true)
          .append('svg')
          .attr('id', 'mainVizSvg')
          .attr("preserveAspectRatio", "xMinYMin meet")
      ...
      clusterSvg.selectAll()
          ...
          .style("fill", function (d) { return arrayToRGBAString(d.rgb); })
          .style("opacity", 0.8)
          .on('mouseover', (d, i, n) => { tooltipOn$1(d, i, n, text); })
          .on('mouseout', (d, i, n) => { tooltipOff$1(d, i, n); });
  ...
  const tooltip$1 = select('body')
      .append('div')
      .attr('class', 'tooltip')
      .style('opacity', 0);
  function tooltipOn$1(d, i, n, text) {
      tooltip$1.style('width', Math.floor($("#mainVizSvg")[0].getBoundingClientRect().width) + 'px');
      const currentEl = n[i];
      ...
      select(currentEl).classed('selected', true);
      selectAll(`rect[cluster='${d.cluster}'][window='${d.window_size}']`)
          .transition()
          .duration(200)
          .style('opacity', 1.0)
          .style("stroke", "black")
          .style("stroke-width", "1px");
      tooltip$1.transition()
          .duration(200)
          .style('opacity', .8);
      const selection_text = text.slice(d.x_start, d.x_start + d.section_length).join(" ");
      tooltip$1.html(selection_text)
          .style('left', gX + 'px')
          .style('top', gY + 'px')
          .style('border-color', arrayToRGBAString(d.rgb));
  }
  function tooltipOff$1(d, i, n) {
      const currentEl = n[i];
      select(currentEl).classed('selected', false);
      selectAll(`rect[cluster='${d.cluster}'][window='${d.window_size}']`)
          .transition()
      ...
      tooltip$1.transition().duration(500).style('opacity', 0);
  }
  ...
  let x = linear$1().range([0, width]).domain([numLevels, 0]);
  let y = linear$1().range([height, 0]);
  select('#fixed')
      .classed("svg-container", true);
  ...
  function updateChapter(data, sentences, currentChapter, chapterBreaks) {
      ...
      rects.enter().append("rect")
          ...
          selectAll("rect[window='" + d.window_size + "']").data().forEach(function (d) {
              $('span').slice(d.x_start, d.x_start + d.section_length).css("color", arrayToRGBAString(d.rgb, 1));
          });
          selectAll("rect[window='" + d.window_size + "']").style("opacity", 1.0);
          selectAll("rect:not([window='" + d.window_size + "'])").style("opacity", 0.8);
      })
      ...
      selectAll("rect[window='" + (numLevels - 1) + "']").data().forEach(function (d) {
          $('span').slice(d.x_start, d.x_start + d.section_length).css("color", arrayToRGBAString(d.rgb, 1));
      });
      selectAll("rect[window='" + (numLevels - 1) + "']").style("opacity", 1.0);
      selectAll("rect:not([window='" + (numLevels - 1) + "'])").style("opacity", 0.8);
  ...
  function init$1(data, sentences, paragraphBreaks, chapterBreaks) {
      selectAll("svg > *").remove();
      Waypoint.destroyAll();
      clusterSvg = select('svg').append('g')
          .attr("class", "focus");
   ...
   */
  let currentLevel;
  function drawZoomPlot(data, text, chapterBreaks) {
      const num_levels = 3;
      ...
      clusterSvg.selectAll()
          ...
          .style("fill", function (d) { return arrayToRGBAString(d.rgb); })
          .style("opacity", 0.8)
          .on('mouseover', (d, i, n) => { tooltipOn$2(d, i, n, text); })
          .on('mouseout', (d, i, n) => { tooltipOff$2(d, i, n); });
      clusterSvg.call(zoom().on("zoom", function () {
  ...
  const tooltip$2 = select('body')
      .append('div')
      .attr('class', 'tooltip')
      .style('opacity', 0);
  function tooltipOn$2(d, i, n, text) {
      tooltip$2.style('width', Math.floor($("#mainVizSvg")[0].getBoundingClientRect().width) + 'px');
      const currentEl = n[i];
      const gX = $("#mainVizSvg")[0].getBoundingClientRect().x;
      const gY = $("#mainVizSvg")[0].getBoundingClientRect().y + $("#mainVizSvg")[0].getBoundingClientRect().height;
      select(currentEl).classed('selected', true);
      selectAll(`rect[cluster='${d.cluster}'][window='${d.window_size}']`)
          ...
          .duration(200)
          .style('opacity', 1.0);
      tooltip$2.transition()
          .duration(200)
          .style('opacity', .8);
      const selection_text = text.slice(d.x_start, d.x_start + d.section_length).join(" ");
      tooltip$2.html(selection_text)
          .style('left', gX + 'px')
          .style('top', gY + 'px')
          .style('border-color', arrayToRGBAString(d.rgb));
  }
  function tooltipOff$2(d, i, n) {
      const currentEl = n[i];
      select(currentEl).classed('selected', false);
      selectAll(`rect[cluster='${d.cluster}'][window='${d.window_size}']`)
          .transition()
          ...
          .style('opacity', 0.8)
          .style("stroke-width", "0px");
      tooltip$2.transition().duration(500).style('opacity', 0);
  }
  ...
  const numLevels$1 = 3;
  let spanPositions = [];
  const height$1 = 700, width$1 = 500;
  const margin = { top: 10, right: 10, bottom: 10, left: 20 };
  let x$1 = linear$1().range([width$1, margin.left]).domain([0, numLevels$1]);
  let y$1 = linear$1().range([height$1, 0]);
  let ignoreScrollEvents = false;
  let currentWindow;
  const mainSvg$1 = select("div#svg-container")
      .append("svg")
      .attr("viewBox", `0 0 ${width$1} ${height$1}`)
      .classed("svg-content-responsive", true);
  let clusterSvg$1;
  function createPlot(data, chapterBreaks, sentences) {
      clusterSvg$1 = mainSvg$1.append('g').attr("id", "clusterSvg");
      // Add line denoting chapter breaks
      const chapterAxis = clusterSvg$1.append("line")
          .style("stroke", "black")
          .attr("x1", 5)
          .attr("y1", y$1(0))
          .attr("x2", 5)
          .attr("y2", height$1);
      const chapterDots = clusterSvg$1.selectAll("circle").data(chapterBreaks)
          .enter().append("circle")
          .style("fill", "black")
          .attr("r", 2)
          .attr("cx", 5)
          .attr("cy", function (d) { return y$1(d); });
      // Add columns with section data
      const rects = clusterSvg$1.selectAll("rect.text-rect").data(data);
      rects.exit().remove();
      rects.enter().append("rect")
          .merge(rects)
          .attr("class", "text-rect")
          .attr("y", function (d) { return y$1(d.x_start); })
          .attr("x", function (d) { return x$1(d.window_size + 1); })
          .attr("height", function (d) { return y$1(d.section_length); })
          .attr("width", 0.99 * (x$1(0) - x$1(1)))
          .attr("cluster", function (d) { return d.cluster; })
          .attr("window", function (d) { return d.window_size; })
          .attr("start", function (d) { return d.x_start; })
          .attr("end", function (d) { return d.x_start + d.section_length; })
      ...
      rects.enter().append("text")
          .style("fill", "white")
          .attr("x", function (d, i) {
          return x$1(d.window_size + 1) + 5;
      })
          .attr("y", function (d) {
          return y$1(d.x_start + d.section_length / 2);
      })
      ...
      $("span.sentence").mouseover(function (event) {
          ...
          selectAll("rect[window='" + selectedSection.window_size + "']").filter(function (d) {
              return d.cluster == selectedSection.cluster;
          }).style("stroke", "black").style("stroke-width", "1px");
      }).mouseout(function (event) {
          selectAll("rect[window='" + currentWindow + "']").style("stroke", "none");
          const sectionId = parseInt(event.currentTarget.getAttribute("section-id"));
          ...
          selectAll("rect[window='" + selectedSection.window_size + "']").filter(function (d) {
              ...
              $("#innerModal").append(`<div class='info-text book-exerpt' 
                                      section-id='${d.id}'
                                      style='color:${arrayToRGBAString(d.rgb)}; margin-bottom: 5px;'>
                                      ${topSentenceString}
                                      </div>`);
          });
          $(".book-exerpt").mouseover(function (subEvent) {
              ...
              select(`rect[section-id='${selectedExerptSectionId}']`)
                  .style("stroke", "black")
                  .style("stroke-width", "1px");
      ...
      $("#main").off('scroll').on('scroll', function () {
          ...
          select(".brush").call(brush.move, [y$1(newExtent[0]), y$1(newExtent[1])]);
      });
      rects.transition().duration(300);
  }
  ...
  function markTopWordsAndSentences(newWindow) {
      ...
      selectAll("rect[window='" + newWindow + "']").data().forEach(function (d) {
          for (let section_idx = d.x_start; section_idx < d.x_start + d.section_length; section_idx++) {
              ...
              for (let word_idx = 0; word_idx < top_words.length; word_idx++) {
                  ...
                  section_html = section_html.replace(regex_word, function (match) {
                      return `<span style='border-color:${arrayToRGBAString(d.rgb, 0.5)}' class='underline'>${match}</span>`;
                  });
              }
  ...
  function colorText(newWindow) {
      selectAll("rect[window='" + currentWindow + "']").data().forEach(function (d) {
          $('span.sentence').slice(d.x_start, d.x_start + d.section_length)
              .css("color", arrayToRGBAString(d.rgb, 1))
  ...
  function updateLevel(direction) {
      if (direction == 'prev') {
          if (currentWindow == numLevels$1 - 1) {
              return;
          }
          currentWindow += 1;
      }
  ...
  function brushed() {
      ...
      const firstSentence = Math.floor(y$1.invert(selection[0]));
  ...
  function fillText$1(sentences, paragraphBreaks, chapterBreaks) {
      $("#container").append(`
  ...
  function init$2(data, sentences, paragraphBreaks, chapterBreaks) {
      selectAll("svg > *").remove();
      ...
      y$1.domain([sentences.length, 0]);
      console.log(y$1.domain);
      fillText$1(sentences, paragraphBreaks, chapterBreaks);
  ...
  function displayData(bookName, vizType, splitBy, embType, dimensions, order, data, idxBookMap) {
      return __awaiter(this, void 0, void 0, function* () {
          ...
          if (vizType === 'grid-plot') {
              ...
              drawGridPlot(bookIdx, embData, embLength, splitBy, dimensions, order, data);
          }
              ...
              drawClusterPlot(cluster_data, text, chapterBreaks);
          }
              ...
              const text = data[bookIdx]['sentences'];
              const chapterBreaks = data[bookIdx]['chapter_breaks_by_sentence'];
              drawZoomPlot(cluster_data, text, chapterBreaks);
          }
              ...
              init$1(cluster_data, sentences, paragraphBreaks, chapterBreaks);
          }
              ...
              const chapterBreaks = data[bookIdx]['chapter_breaks_by_sentence'];
              const paragraphBreaks = data[bookIdx]['paragraph_breaks'];
              init$2(cluster_data, sentences, paragraphBreaks, chapterBreaks);
          }
      });
  }
  main();
