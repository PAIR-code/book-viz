...
const numLevels = 3;
const height: number = 800, width: number = 300;
...
function updateChapter(data: any, sentences: string[], currentChapter: number, chapterBreaks: number[]) {
    const chapterData = data.filter(function (entry: any) {
    ...
    rects.enter().append("rect")
         ...
         .attr("x", function(d: any) { return x(d.window_size + 1) + 0.05*(x(0) - x(1)) })
         ...
         .on("click", function(d: any, i: any) {
            d3.selectAll("rect[window='"+ d.window_size +"']").data().forEach(
                function(d: any) {
    ...
    d3.selectAll("rect[window='"+ (numLevels - 1) +"']").data().forEach(
        function(d: any) {
            $('span').slice(d.x_start, d.x_start + d.section_length).css("color", utils.arrayToRGBAString(d.rgb, 1))
        })
...
function setOnScroll(n: string, y: any) { 
    let startingSpanIdx: number = ($("#" + n + " span").offset().top > 0) ? 0 : -1
    let currentSpanId: string = $("#" + n + " span").get(startingSpanIdx).id;
    const midPoint = $("#main").height()/2 + $("#main").position().top
    let distanceToMidpoint = (id: string) => Math.abs($("span#" + id).offset().top - midPoint)
...
function scroll(n: string, offset: string, func: any, prevIdx: number, 
    nextIdx: number, data: any, sentences: string[], chapterBreaks: number[]){
