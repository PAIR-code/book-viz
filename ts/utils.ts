...
/**
 * @fileoverview Helper functions for processing and representing data.
 */
 ...
 * */
export function getParData(
    paragraphs: string[], chapterBreaks: number[], embeddings: number[][],
    order: string, dimensions: string) {
  if (order == 'color') {
...
function getPerChapterJson(
    paragraphs: string[], chapterBreaks: number[],
    embeddings: number[][]): ChapterText[] {
  // create parData, where each entry corresponds to a paragraph,
  // and is a json of form {value = chapter #, text = paragraph text}
  let parData: ChapterText[] = new Array(paragraphs.length);

  for (let chapter = 0; chapter < chapterBreaks.length; chapter++) {
    const nextChapterParCutoff =
        ((chapter === chapterBreaks.length - 1) ? paragraphs.length :
...
function getPerColorJson(
    paragraphs: string[], embeddings: number[][],
    dimension: string): ChapterText[] {
  let textColorData = [];
  for (let i = 0; i < paragraphs.length; i++) {
    textColorData.push(
        {'text': paragraphs[i], 'rgb': embeddings[i], 'hsl': embeddings[i]})
  }
...
function sortTextByColor(
    textColorData: {text: string, rgb: number[], hsl: number[]}[],
    dimension: string) {
  let sortedData;
  if (dimension == 'all') {
    ...
    sortedData = textColorData.sort(
        (c1: {text: string, rgb: number[], hsl: number[]},
          c2: {text: string, rgb: number[], hsl: number[]}) => {
          // Sort by hue
...
export function arrayToRGBAString(arr: number[], opacity: number = 0.8) {
  return (
      'rgba(' + arr[0] * 255 + ',' + arr[1] * 255 + ',' + arr[2] * 255 + ',' + opacity + ')');
}