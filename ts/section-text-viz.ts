...
const numLevels = 3;
let spanPositions: number[] = [];
const height: number = 700, width: number = 500;
const margin: {top: number; right: number; bottom: number;
    left: number;} = {top: 10, right: 10, bottom: 10, left: 20};
...
function createPlot(data: any, chapterBreaks: number[], sentences: string[]) {
    ...
    const chapterDots = clusterSvg.selectAll("circle").data(chapterBreaks)
                                  ...
                                  .attr("r", 2)
                                  .attr("cx", 5)
                                  .attr("cy", function(d: any) { return y(d)});
    ...
    rects.enter().append("rect")
         ...
         .attr("y", function(d: any) { return y(d.x_start)})
         .attr("x", function(d: any) { return x(d.window_size + 1) })
         .attr("height", function(d: any) { return y(d.section_length) })
         .attr("width", 0.99*(x(0) - x(1)))
         .attr("cluster", function(d: any) {return d.cluster})
         .attr("window", function(d: any) {return d.window_size})
         .attr("start", function(d: any) {return d.x_start})
         .attr("end", function(d: any) {return d.x_start + d.section_length})
         .attr("section-id", function(d: any) {return d.id})
    ...
    rects.enter().append("text")
        ...
        .attr("window_size", function (d: any) {return d.window_size})
        .attr("content", function (d: any) {return getTopWordForDisplay(d, data)})
        .style("style", "label")
        ...
        .text(function (d: any) { 
            ...
            if (topWord != "") {allLabels.push(topWord)}
            return topWord
        });
    ...
    $("span.sentence").mouseover(function(event: MouseEvent) {
        const sectionId = parseInt((<HTMLSpanElement>event.currentTarget).getAttribute("section-id")!)
        console.log(event.currentTarget)
        console.log(data[sectionId])
        const selectedSection = data[sectionId]
        d3.selectAll("rect[window='"+ selectedSection.window_size +"']").filter(function(d: any) {
        ...
        d3.selectAll("rect[window='"+ selectedSection.window_size +"']").filter(function(d: any) {
          ...
          $(".book-exerpt").mouseover(function(subEvent: MouseEvent) {
            const selectedExerptSectionId = parseInt((<HTMLSpanElement>subEvent.currentTarget).getAttribute("section-id")!)
            d3.select(`rect[section-id='${selectedExerptSectionId}']`)
              .style("stroke", "black")
              .style("stroke-width", "1px")
          }).mouseout(function(subEvent: MouseEvent) {
            const selectedExerptSectionId = parseInt((<HTMLSpanElement>subEvent.currentTarget).getAttribute("section-id")!)
            d3.select(`rect[section-id='${selectedExerptSectionId}']`)
              .style("stroke", "none")
          })
...
function getTopWordForDisplay(d: any, data: any) {
...
function markTopWordsAndSentences(newWindow: number) {
    currentWindow = newWindow
    d3.selectAll("rect[window='"+ newWindow +"']").data().forEach(
        function(d: any) {
            for (let section_idx = d.x_start; section_idx < d.x_start + d.section_length; section_idx++) {
                ...
                for (let word_idx = 0; word_idx < top_words.length; word_idx++) {
                    ...
                    section_html = section_html.replace(regex_word, function(match: string) {
...
function colorText(newWindow: number) {
    d3.selectAll("rect[window='"+ currentWindow +"']").data().forEach(
        function(d: any) {
...
function updateLevel(direction: string) {
    if (direction == 'prev') {
      if (currentWindow == numLevels - 1) { return; }
      currentWindow += 1
    } else {
...
function fillText(sentences: string[], 
...
export function init(
    ...
    chapterBreaks: number[]) {
        ...
        $(window).click(function(event: MouseEvent) {
